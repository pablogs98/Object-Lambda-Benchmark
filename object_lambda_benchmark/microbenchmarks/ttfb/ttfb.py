...
handlers = ["lambda_function.lambda_handler", "index.handler"]

for f, function_ in enumerate(lambda_functions):
    olambda_function = ObjectLambdaFunction(function_name=function_)
    lambda_function = LambdaFunction(function_name=function_)

    # Creates both! Lambda and Object lambda (same function). Also creates Object Lambda's endpoint
    olambda_function.create_function(handler=handlers[f], runtime=runtimes[f], role='pablo-execution-role',
                                     zip_file_path=zip_paths[f], s3_access_point='pablo-data-ap')
    ...
    for m_size in memory_sizes:
        ...
        for object_size in object_sizes:
            ...
            with open(f'./results/{now.day}/{now.hour}/ttfb_{runtimes_no_number[f]}_{object_size}_{m_size}mb.json',
                      'w') as results_file:
    ...
    # Deletes all (Lambda, Object Lambda)
